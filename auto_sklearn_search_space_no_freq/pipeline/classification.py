...
from auto_sklearn_search_space_no_freq.pipeline.base import BasePipeline
from auto_sklearn_search_space_no_freq.pipeline.components.classification import ClassifierChoice
from auto_sklearn_search_space_no_freq.pipeline.components.data_preprocessing import DataPreprocessorChoice
from auto_sklearn_search_space_no_freq.pipeline.components.data_preprocessing.balancing.balancing import (
    Balancing,
)
from auto_sklearn_search_space_no_freq.pipeline.components.feature_preprocessing import (
    FeaturePreprocessorChoice,
)
from auto_sklearn_search_space_no_freq.pipeline.constants import SPARSE
...
class SimpleClassificationPipeline(BasePipeline, ClassifierMixin):
    ...
    _preprocessor : The underlying scikit-learn processing algorithm. This
    ...
    def rearrange_probas(self, preds, labels):
        if preds.shape[1] != len(labels):  
            y_pred_new = np.zeros((preds.shape[0], len(labels)))
            col_indices = [labels.tolist().index(label) for label in self.get_class_order()]
            y_pred_new[:, col_indices] = preds
            preds = y_pred_new
