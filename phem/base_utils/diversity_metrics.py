...
def average_loss_correlation(
    y_true, Y_pred_base_models: list[np.ndarray], weights=None, aggregation_method=np.mean, abs=True,
):
    ...
    corr_matrix = np.corrcoef(loss_per_bm)
    if abs:
...
def average_loss_correlation_regression(
    y_true, Y_pred_base_models: list[np.ndarray], weights=None, aggregation_method=np.mean, abs=True
):
    """
    Average Loss Correlation (ALC) for regression.
    Measures the correlation between the loss vectors (per-instance errors)
    ...
    Y_pred_base_models : list of np.ndarray
        Predictions from base models. Each element is of shape (n_samples,).
    weights : optional
        Currently unused. Placeholder to mirror classification signature.
    aggregation_method : callable
        Function to aggregate the pairwise correlations (default: np.mean).

    Returns
    -------
    float
        Average pairwise loss correlation across base models.
    """

    # Convert to 1D array if needed
    y_true = np.asarray(y_true).ravel()

    # Compute per-model loss vectors (using squared error)
    loss_per_bm = [
        y_true - bm_pred.ravel() for bm_pred in Y_pred_base_models
    ]

    # Stack into matrix: shape = (n_models, n_samples)
    loss_matrix = np.vstack(loss_per_bm)

    # Pairwise correlation matrix across models
    corr_matrix = np.corrcoef(loss_matrix)
    if abs:
        corr_matrix = np.abs(corr_matrix)

    # Keep only the upper triangle (pairwise correlations)
    upper = corr_matrix[np.triu(np.ones(corr_matrix.shape), k=1).astype(bool)]

    # Replace NaNs (e.g., from constant predictions) with 0
    upper = np.nan_to_num(upper)
...
def get_loss_correlation_metric(is_classification, abs=True):
    if is_classification:
        return make_diversity_metric(
            "OrdinalClassification", "Average Loss Correlation (Classification)", partial(average_loss_correlation, abs=abs)
        )
    else:
        return make_diversity_metric(
            "NonOrdinalClassification", "Average Loss Correlation (Regression)", partial(average_loss_correlation_regression, abs=abs)
        )