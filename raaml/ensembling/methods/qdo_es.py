...
class QDOEnsembleSelection(AbstractEnsemblingStrategy):
    """QDO Ensemble Selection."""
    ...
    def __init__(
        ...
        self.normalize_predict_proba_ = False # Originally in AbstractWeightedEnsemble.init()
        self.model_specific_metadata_ = {} # Originally in AbstractWeightedEnsemble.init()
    ...
    def _setup_metadata(self, inference_times, energies, configs, config_ids, threads, frequencies):
        if self.behavior_space.requires_base_model_metadata:
            
            self.base_models_metadata = [{
                "task": Task(
                    c_id, 
                    threads[i], 
                    frequencies[i], 
            ...
            #self.base_models_metadata = [{key: res[i] for i, key in enumerate(resource_names)} for res in resources]
            for i in range(len(self.base_models_metadata)):
                self.base_models_metadata[i]["config"] = configs[i]
                self.base_models_metadata[i]["auto-sklearn-model"] = "dummy"
            ...
            if self.base_models_metadata_type == "custom":
                ...
                def is_numeric(x):
                    return isinstance(x, numbers.Number) or np.issubdtype(type(x), np.number)
    ...
    def ensemble_fit(
        ...
        inf_times_index = resource_names.index("inference_time")
        energy_index = resource_names.index("energy_consumption")
        inference_times, energies = resource_consumptions[:, inf_times_index], resource_consumptions[:, energy_index]

        self._setup_metadata(inference_times, energies, configs, config_ids, threads, frequencies)
        ...
        # -- Init QDO Elements
        self._init_archive()
        self._init_emitters(predictions, y_true)

        # -- Call optimizer
        self._optimize(predictions, y_true)

        # -- Get final weights
        self._compute_weights(predictions, y_true)
    ...
    def _compute_weights(self, predictions, labels):
        ...
        self.weight_matrix = np.array([e.sol for e in elites])
...
def evaluate_single_solution(
    ...
    if raw_req:
        pred_arguments["raw_preds"] = [
            QDOEnsembleSelection._confidences_to_predictions(y_pred_ensemble),
