...
for n in {1..8}; do
    ...
    srun -c $n --cpu-bind=cores python -u -m experiments.run_create_preds --amlb_task_id_zero_based "${task_index}" --bmg_method "${method}" --n_threads $n --seed $seed --n_cpu_filter 1
done
for n in {12..64..4}; do
    echo "Running with $n cores..."
    srun -c $n --cpu-bind=cores python -u -m experiments.run_create_preds --amlb_task_id_zero_based "${task_index}" --bmg_method "${method}" --n_threads $n --seed $seed --n_cpu_filter 1
