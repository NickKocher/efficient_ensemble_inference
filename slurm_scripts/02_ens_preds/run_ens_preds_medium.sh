...
#SBATCH --output=./logs/ens_preds_low/%A_%a.out
#SBATCH --array 0-5
#SBATCH --exclusive
...
for n in {1..8}; do
    ...
    srun -c $n --cpu-bind=cores python -u -m experiments.run_create_preds --amlb_task_id_zero_based "${task_index}" --bmg_method "${method}" --n_threads $n --seed $seed --n_cpu_filter 2
done
for n in {12..64..4}; do
    echo "Running with $n cores..."
    srun -c $n --cpu-bind=cores python -u -m experiments.run_create_preds --amlb_task_id_zero_based "${task_index}" --bmg_method "${method}" --n_threads $n --seed $seed --n_cpu_filter 2
