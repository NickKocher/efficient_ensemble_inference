...
class SelectClassificationRates(AutoSklearnPreprocessingAlgorithm):
    def __init__(self, alpha, frequency, mode="fpr", score_func="chi2", random_state=None):
    ...
    def get_hyperparameter_search_space(
        ...
        # Why not add all at once?
        
        cs.add_hyperparameters([alpha, score_func, mode, frequency])

        # mutual_info_classif constantly crashes if mode is not percentile
        # as a WA, fix the mode for this score
        cond = InCondition(mode, score_func, ["chi2", "f_classif"])
