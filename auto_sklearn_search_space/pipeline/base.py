...
import auto_sklearn_search_space.pipeline.create_searchspace_util as create_searchspace_util
from auto_sklearn_search_space.askl_typing import FEAT_TYPE_TYPE

from auto_sklearn_search_space.pipeline.components.regression.extra_trees import ExtraTreesRegressor
from auto_sklearn_search_space.pipeline.components.classification.extra_trees import ExtraTreesClassifier
from auto_sklearn_search_space.pipeline.components.regression.random_forest import RandomForest as RandomForestRegressor
from auto_sklearn_search_space.pipeline.components.classification.random_forest import RandomForest as RandomForestClassifier
from auto_sklearn_search_space.pipeline.components.regression.k_nearest_neighbors import KNearestNeighborsRegressor
from auto_sklearn_search_space.pipeline.components.classification.k_nearest_neighbors import KNearestNeighborsClassifier
from auto_sklearn_search_space.pipeline.components.feature_preprocessing.random_trees_embedding import RandomTreesEmbedding
from auto_sklearn_search_space.pipeline.components.feature_preprocessing.extra_trees_preproc_for_classification import ExtraTreesPreprocessorClassification
from auto_sklearn_search_space.pipeline.components.feature_preprocessing.extra_trees_preproc_for_regression import ExtraTreesPreprocessorRegression

multi_threading_list_estimator = (
    ExtraTreesRegressor, ExtraTreesClassifier, RandomForestRegressor, RandomForestClassifier,
    KNearestNeighborsRegressor, KNearestNeighborsClassifier,
)

multi_threading_list_preprocessor = (
    RandomTreesEmbedding, ExtraTreesPreprocessorClassification, ExtraTreesPreprocessorRegression,
)
...
class BasePipeline(Pipeline):
    ...
    def allows_for_multi_threading_inference(self):
        ret = False
        
        for step in self.steps:
            if step[0] == "feature_preprocessor":
                feat_preproc = step[1]
            if step[0] in ["classifier", "regressor"]:
                estimator = step[1]
        
        if isinstance(feat_preproc.choice, multi_threading_list_preprocessor):
            ret = True
            
        if isinstance(estimator.choice, multi_threading_list_estimator):
            ret = True
            
        return ret
    
    def get_class_order(self):
        if "classifier" not in self.named_steps.keys():
            raise ValueError("Cannot get class order, because no classifier is present in the pipeline.")
        classifier = self.named_steps["classifier"].choice
        if not hasattr(classifier.estimator, "classes_"):
            raise ValueError("The classifier does not have the 'classes_' attribute.")
        return classifier.estimator.classes_
    
    def set_frequencies(self, frequencies: Dict[str, float]):
        steps_dict = {arr[0]: arr[1] for arr in self.steps}
        for key, value in frequencies.items():
            step_str_arr = key.split(":")
            step = steps_dict[step_str_arr[0]]
            if step_str_arr[0] in ["classifier", "regressor", "feature_preprocessor"]:
                choice = step.choice
                if hasattr(choice, "frequency"):
                    choice.frequency = value
                else:
                    print("Warning: The choice {} does not have a frequency attribute.".format(choice))
            elif step_str_arr[0] == "data_preprocessor":
                data_preproc_step = step_str_arr[2]
                if data_preproc_step == "numerical_transformer":
                    ppl = step.choice.numer_ppl
                elif data_preproc_step == "categorical_transformer":
                    ppl = step.choice.categ_ppl
                elif data_preproc_step == "text_transformer":
                    ppl = step.choice.text_ppl
                
                ppl_steps_dict = {arr[0]: arr[1] for arr in ppl.steps}
                ppl_step = ppl_steps_dict[step_str_arr[3]]

                if hasattr(ppl_step, "frequency"):
                    ppl_step.frequency = value
                elif isinstance(ppl_step, AutoSklearnChoice):
                    if hasattr(ppl_step.choice, "frequency"):
                        ppl_step.choice.frequency = value
                    else:
                        print("Warning: The choice {} does not have a frequency attribute.".format(ppl_step.choice))
                else:
                    print("Warning: The step {} does neither have a frequency attritbute nor is a Choice".format(ppl_step))
            elif step_str_arr[0] == "balancing":
                if hasattr(step, "frequency"):
                    step.frequency = value
                else:
                    print("Warning: The step {} does not have a frequency attribute.".format(step))
            else:
                print("Warning: The step {} is neither classifier, regressor, feature_preprocessor nor data_preprocessor".format(step_str_arr[0]))
